...
const kafka = new Kafka({
  clientId: 'graphql-logs-integrations',
  brokers: [process.env.BROCKER_HOST]
})

const consumer = kafka.consumer({ groupId: 'logs-group-integrations' })
...
const run = async () => {
  ...
  await consumer.subscribe({ topic: 'logs.integrations', fromBeginning: true })
